...
$registros=mysql_query("select nombre,apellido,telefono from usuario where nick='$_SESSION[nickk]'",$conexion) or
  die("Polemas en el select1:".mysql_error());
  ...
  #
  $fecha2=date_default_timezone_set("America/Chicago");
$fecha2=strftime( "%d-%m-%Y", time() );
...
$c=$dd[2];
  #
   if($a>=$dia and $b>=$mes and $c>=$anyo and $nombre != "" and $apellido != "" and $_REQUEST[fecha] != "" and $_REQUEST[ox] != "" and $_REQUEST[hora] != ""){
$registros=mysql_query("insert into visitantes(nombre,apellido,telefono,fecha,hora,servicio) values ('$nombre','$apellido','$telefono','$dd2','$_POST[hora]','$_POST[ox]')",$conexion) or
  die("Probleas e el select1:".mysql_error());
  #punciona ;) 3 hora en eto
  echo "Gracias su visita se tomara en cuenta";
  } else {echo "La fecha ".$dd." ya paso";
     }
...
?>
    ...
    <form action="sitar_.php" method="post" class="registro">
<div>
  <label></label>
</div>
<div>
...
<input type="submit" name="enviar" value="Ingresar"></div>
      ...
      </table><h2>Ten en cuenta que los servicios los damos por orden de llegada aunque ya aigas ingresado tu fecha de visita primero, este servicio nos ayuda basicamente a saber la posible cantidad de clientes que tendremos.</h2></div>
</div>
